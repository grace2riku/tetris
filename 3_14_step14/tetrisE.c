...
void deleateLine(int ys);   // 揃った行を一行マップから消滅させる
int deleateMap(void);      // 揃った行を全部マップ上から消去する
...
int checkCell(Cell a, int x, int y) {
    if (checkRenge(a, x, y) || map[y][x].c != '\0')
        return -1;  // 失敗
    return 0;       // 成功
}
...
int checkLine(int y){
    ...
    for (x = 0; x < WIDTH; x++)
        if (map[y][x].c == '\0')    // どこか一箇所でも空セルがあれば失敗
            return -1;
    return 0;
}

// 揃った行を一行マップから消滅させる
void deleateLine(int ys){
    int x, y;

    for(y = ys; y > 0; y--)             // 対象ラインより上のラインの繰り返し
        for (x = 0; x < WIDTH; x++)     // 一行の繰り返し
            map[y][x] = map[y-1][x];    // 一つ上のセルを下に落とす
    setBackColor(BLACK);
    clearScreen();                      // ゲーム画面をクリア
...
int deleateMap(void){
    ...
    for (y=0; y < HEIGHT; y++)  // ゲーム画面の全部の行を繰り返し
        if (checkLine(y) == 0) {  // 揃ったラインがあれば一行消滅
            deleateLine(y);
            count++;
        }
    return count;
...
{
    int x, y, c, count, prex, prey, t, next, score;
    ...
    for ( ; ; ) {
        ...
        if (duration > thold) { 
            ...
            if (checkMap(block, x, y+1) == 0)
                y++;    // ブロックが落下し終わっていなければ一つ落下
            else {
                int line;

                if (y == BLOCK_SIZE) { // yが天井だったら修了処理
                    reset();
                    exit(1);
                }
                // ブロックを積み上げる(x, y変更前におこなう)
                putMap(block, x, y);
